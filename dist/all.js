...
(function (window, document, undefined) {
  ...
  function ColorPicker(slideElement, pickerElement, callback) {
    ...
    if (type == "SVG") {
      ...
      var blackAndWhiteGradients = [
        gradientDefs.firstChild,
        gradientDefs.lastChild,
      ];
      ...
      whiteAndBlackRects[0].setAttribute(
        "fill",
        "url(#" + blackAndWhiteGradients[1].id + ")"
      );
      whiteAndBlackRects[1].setAttribute(
        "fill",
        "url(#" + blackAndWhiteGradients[0].id + ")"
      );
   ...
   */
  ColorPicker.positionIndicators = function (
    slideIndicator,
    pickerIndicator,
    mouseSlide,
    mousePicker
          ...
          return (
            (a = a.replace(/(?:^|\s)(\w+)\(([^)]+)\)/g, function (a, c, d) {
              return (
                ...
                "skewX" == c
                  ? b.push(["m", 1, 0, N.tan(l(d[0])), 1, 0, 0])
                  : "skewY" == c
                  ? b.push(["m", 1, N.tan(l(d[0])), 0, 1, 0, 0])
...
function createChar(evt) {
  ...
  if (!currentUser.cc.personnageActuel) {
    currentUser.cc.personnageActuel = personnageActuel;
  }
  if (!currentUser.cc.personnages) {
    currentUser.cc.personnages = {};
  }
...
function deleteChar() {
  var el = this;
  var disposible = el.parentNode.parentNode.querySelector(
    ".overlay__char-name"
  ).innerHTML;
...
function saveChar() {
  ...
  updateDbUser(currentUser).then(function (json) {
    currentUser._rev = json.rev;
    // return json
    gaga("send", "event", {
      eventCategory: "Navigation",
      eventAction: "Save",
      eventLabel: "Save character",
    });
  });
  /*
    .catch(function (err) {
...
function colorElement(el) {
  ...
  if (
    (processSection(section, item) === "skin" ||
      processSection(section, item) === "mouth") &&
    c.choices.skinColor != undefined
  ) {
...
function colorElementLoop(el, colorPrefix, newColor, lipstickFlag) {
  ...
  if (counter > 0) {
    ...
    while (counter--) {
      childrenList[counter] = applyColorToChild(
        childrenList[counter],
        colorPair
      );
    }
  }
  ...
  while (contrastCounter--) {
    childrenList = el.querySelectorAll(
      classPrefix + colorPrefix + "--" + colorContrasts[contrastCounter]
    );
    counter = childrenList.length;

    if (counter > 0) {
      if (colorContrasts[contrastCounter] === "light") {
        colorListIndex = 4;
      }
      if (colorContrasts[contrastCounter] === "dark") {
        colorListIndex = 2;
      }
    ...
    while (suffixCounter--) {
      childrenList = el.querySelectorAll(
        "." +
          colorPrefix +
          "--" +
          colorContrasts[contrastCounter] +
          colorSuffixes[suffixCounter]
      );
      counter = childrenList.length;

      if (counter > 0) {
        if (
          colorContrasts[contrastCounter] + colorSuffixes[suffixCounter] ===
          "darkest"
        ) {
          colorListIndex = 0;
        }
...
function applyColorToChild(child, colorPair) {
  if (child.style.fill != "none" && child.style.fill != "") {
    child.style.fill = colorPair[0];
  }
  if (child.style.stroke != "none" && child.style.stroke != "") {
    child.style.stroke = colorPair[1];
  }
...
function colorize(formId, _color) {
  ...
  var palletteButton = document.querySelector(
    '.section-pallette input[type="radio"]:checked+label'
  );
  ...
  for (var f in forms) {
    ...
    for (var x in form) {
      ...
      if (form[x].toLowerCase() === id) {
        ...
        var capitalId = id.replace(/^[a-z]/, function (m) {
        ...
        for (n in affectedList) {
          ...
          var alphaNodes = document.querySelectorAll(
            fullId + seperator + classPrefix
          );
          var alphaLightNodes = document.querySelectorAll(
            fullId + seperator + classPrefix + classLight
          );
          var alphaLighterNodes = document.querySelectorAll(
            fullId + seperator + classPrefix + classLighter
          );
          var alphaLightestNodes = document.querySelectorAll(
            fullId + seperator + classPrefix + classLightest
          );
          var alphaDarkNodes = document.querySelectorAll(
            fullId + seperator + classPrefix + classDark
          );
          var alphaDarkerNodes = document.querySelectorAll(
            fullId + seperator + classPrefix + classDarker
          );
          var alphaDarkestNodes = document.querySelectorAll(
            fullId + seperator + classPrefix + classDarkest
          );
          ...
          while (alphaLightNodesCounter--) {
            colorPaths(
              alphaLightNodes[alphaLightNodesCounter],
              colorLight,
              colorDark
            );
          }
          ...
          while (alphaLighterNodesCounter--) {
            colorPaths(
              alphaLighterNodes[alphaLighterNodesCounter],
              colorLighter,
              _color
            );
          }
          ...
          while (alphaLightestNodesCounter--) {
            colorPaths(
              alphaLightestNodes[alphaLightestNodesCounter],
              colorLightest,
              colorLight
            );
          }
          ...
          while (alphaDarkNodesCounter--) {
            colorPaths(
              alphaDarkNodes[alphaDarkNodesCounter],
              colorDark,
              colorDarker
            );
          }
          ...
          while (alphaDarkerNodesCounter--) {
            colorPaths(
              alphaDarkerNodes[alphaDarkerNodesCounter],
              colorDarker,
              colorDarkest
            );
          }
          ...
          while (alphaDarkestNodesCounter--) {
            colorPaths(
              alphaDarkestNodes[alphaDarkestNodesCounter],
              colorDarkest,
              colorDarkest
            );
          }
...
function parseFormCookie() {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    return JSON.parse(
      decodeURIComponent(val).split(".").slice(0, -1).join(".")
    );
  }
...
function svgTo(type, svg, filename) {
  ...
  img.onload = function () {
    ...
    var svgWidth = img.width;
    var svgHeight = img.height;
...
function getDownloadViewBox() {
  var viewBoxValue;
  var cameraViewContainer = document.querySelector(
    ".camera-view input:checked + label svg"
  );
...
function getSVG() {
  ...
  var text =
    '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"  id="character" width="560" height="560" viewBox="' +
    viewBoxValue +
    '">\n';
  text +=
    '<defs> Copyright (c) 2014-2020 Frederic Guimont, all rights reserved \n <a href="https://creativecommons.org/licenses/by-nc/2.0/">Distributed under the Creative Commons CC-BY-NC license</a> \n Commercial license available to patrons on https://www.patreon.com/charactercreator \n You can recreate this character using this URL: <!--' +
  ...
  svgNodes.forEach(function (item) {
    ...
    if (
      item.innerHTML &&
      (!item.style || !item.style.opacity || item.style.opacity != 0)
    ) {
...
function download(ev) {
  ...
  gaga("send", "event", {
    ...
    eventLabel: "Download SVG file of character",
  });
  ...
  if (format === "png") {
    ...
    return svgToPng(text, filename).then(function () {
      caboose();
    });
  }
...
function initEyes() {
  ...
  function init() {
    ...
    characterHeadCenter.y =
      svgContainerPos.y + characterHeadPos.height * propotionalPlacement; // propotional placement of eyes.
  ...
  function handleMouseMove(event) {
    ...
    if (diffXRightAbs > diffYabs) {
      if (diffXRight > 3 * scaleMod) {
        newXRight = 3;
      } else if (diffXRight < -3 * scaleMod) {
        newXRight = -3;
      } else {
        newXRight = diffXRight / scaleMod;
      }
...
function createForm(sex, forms) {
  ...
  var sectionHtml =
    '<h2 class="sidebar__title"><svg class="icon"><use xlink:href="#icon-coathanger"></use></svg>Categories</h2>';
  ...
  for (var f in forms) {
    ...
    sectionHtml +=
      '<section class="accordeon__section-label' +
      isNewInCategory(sectionNames[f]) +
      '"><span class="accordeon__section-title"><svg class="icon"><use xlink:href="#' +
      getIconId(sectionNames[f], sex) +
      '"></use></svg><span class="accordeon__section-title__text">' +
      sectionNames[f] +
      '</span></span><div class="accordeon__svg-container section-btn--hide"><svg width="1em" height="1em"><use xlink:href="#accordeon_btn"/></svg></div></section><div class="accordeon__content section--hide">';
    ...
    for (var x in forms[f]) {
      sectionHtml +=
        '    <a class="section__link"><li class="sbl__option' +
        isNewInSection(sectionNames[f], x) +
        '" tabindex="0">' +
        x +
        "</li></a>";
      var sectionTitle = x;
      ...
      newHtml +=
        '    <div class="Row options__container options__' +
        t +
        '"><span class="svg__section__title">' +
        t +
        '</span><div class="thumbnails__container">';
      var xsel = hash.get(t);
      var options = forms[f][x]
        .map(function (d, i) {
          var tempId = "#" + t + "_" + d;
          ...
          newHtml +=
            '    <div class="option__container option__' +
            t +
            "_" +
            d +
            newClassName +
            '" tabindex="0"><svg viewBox="' +
            viewBox +
            '" class="svg__option ' +
            t +
            "_" +
...
function replaceMultilayer(layersList, section) {
  ...
  while (multiCounter--) {
    ...
    if (fullList.includes(currentItem)) {
      ...
      while (qtyCounter--) {
        fullList.push(
          currentItem + "_" + (qtyCounter + 1) + "_of_" + currentQty
        );
      }
    }
  }
...
function loadSectionLayers(section, layersList, callback, callbackLoopFlag) {
  ...
  if (section === "emotion") {
    while (layerCounter--) {
      tempLayerList = tempLayerList.concat(
        fromEmotionGetLayers(layersList[layerCounter])
      );
    }
    ...
    while (layerCounter--) {
      tempLayerList = tempLayerList.concat(
        getNailsLayers(layersList[layerCounter])
      );
    }
    layersList = tempLayerList;
    ...
    while (layerCounter--) {
      tempLayerList = tempLayerList.concat(
        bodyTypesToLayers(layersList[layerCounter])
      );
    }
    layersList = tempLayerList;
  } else {
...
function getPositionDir(layer) {
  ...
  if (
    sex === "m" &&
    ((window.maleBody.bodyBack &&
      window.maleBody.bodyBack.indexOf(layer) > -1) ||
      (window.maleBody.bodyMiddle &&
        window.maleBody.bodyMiddle.indexOf(layer) > -1) ||
      (window.maleBody.bodyFront &&
        window.maleBody.bodyFront.indexOf(layer) > -1) ||
      (window.maleBody.bodyOver &&
        window.maleBody.bodyOver.indexOf(layer) > -1))
  ) {
    positionDir = window.maleBodyPositionFolder + "/";
  } else if (
    sex === "m" &&
    ((window.maleHead.headBack &&
      window.maleHead.headBack.indexOf(layer) > -1) ||
      (window.maleHead.headMiddle &&
        window.maleHead.headMiddle.indexOf(layer) > -1) ||
      (window.maleHead.headFront &&
        window.maleHead.headFront.indexOf(layer) > -1) ||
      (window.maleHead.headOver &&
        window.maleHead.headOver.indexOf(layer) > -1))
  ) {
    positionDir = "head_front_default/";
  } else if (
    sex === "f" &&
    ((window.femaleBody.bodyBack &&
      window.femaleBody.bodyBack.indexOf(layer) > -1) ||
      (window.femaleBody.bodyMiddle &&
        window.femaleBody.bodyMiddle.indexOf(layer) > -1) ||
      (window.femaleBody.bodyFront &&
        window.femaleBody.bodyFront.indexOf(layer) > -1) ||
      (window.femaleBody.bodyOver &&
        window.femaleBody.bodyOver.indexOf(layer) > -1))
  ) {
    positionDir = window.femaleBodyPositionFolder + "/";
  } else if (
    sex === "f" &&
    ((window.femaleHead.headBack &&
      window.femaleHead.headBack.indexOf(layer) > -1) ||
      (window.femaleHead.headMiddle &&
        window.femaleHead.headMiddle.indexOf(layer) > -1) ||
      (window.femaleHead.headFront &&
        window.femaleHead.headFront.indexOf(layer) > -1) ||
      (window.femaleHead.headOver &&
        window.femaleHead.headOver.indexOf(layer) > -1))
  ) {
    positionDir = "head_front_default/";
  }
...
function loadFilesFromList(
  layersList,
  callback,
  callbackLoopFlag,
  parentContainer = "#svg1 .character-container"
) {
  ...
  while (counter--) {
    ...
    fetch(file)
      ...
      .then(function (text) {
        var htmlObject = document.createElementNS(
        ...
        if (svgContainer.querySelector("#" + layerID) === null) {
          if (nextLayerSibling != null) {
            nextLayerSibling.parentNode.insertBefore(
              svgObject,
              nextLayerSibling
            );
          } else {
            if (outline) {
              ...
              document
                .querySelector("#svg1 .character-container")
                .appendChild(svgObject);
            }
          }
        }
...
function tagHeadElement(svgObject) {
  ...
  if (
    headElements.includes(elementIdList[0]) ||
    (elementIdList[0] === "body" && elementIdList[1] === "head")
  ) {
...
function closeSections(exception) {
  ...
  while (i--) {
    ...
    if (
      section !== exception &&
      section.parentNode.parentNode.parentNode.classList.contains(
        "sidebar-left"
      )
    ) {
      ...
      if (
        sectionContent.classList === undefined &&
        sectionContent.nextSibling.classList != undefined
      ) {
...
function toggleSection(ev) {
  ...
  if (
    this.parentNode.parentNode.parentNode.classList.contains("sidebar-left")
  ) {
    closeSections(_);
  }
...
function changeOption() {
  ...
  if (
    colors.classList === undefined &&
    colors.previousSibling.classList != undefined
  ) {
...
function manageSelectedItem(userChoice, category) {
  ...
  var sectionNamePart0 = document.querySelector(
    "#sidebar-left .new.section--selected"
  );

  if (!sectionNamePart0) return; // bail out

  var sectionName =
    sectionNamePart0.parentNode.parentNode.previousSibling.firstChild.firstChild.nextSibling.innerHTML.toLowerCase();
  ...
  if (isNew(category, userChoice) !== "" && newItem.classList.contains("new")) {
    ...
    if (newItem.parentNode.querySelectorAll(".new").length === 0) {
      sectionEl = document.querySelectorAll(
        "#sidebar-left .new.section--selected"
      );
...
var proVersion = document.querySelector("flag-version").innerHTML === "true"; // Flag to activation features meant for thecharactercreator.com, switch to true before pushing to the dot com
...
var maleForm3 = {
  ...
  Vest: [
    ...
    "3-button_asymmetric",
    "6-button_asymmetric",
    "2x4-button_double-breasted",
    "2x5-button_double-breasted",
    "2x2-button_double-breasted",
    "2x3-button_double-breasted",
    "5-button_basic",
    "6-button_basic",
    "3-button_basic",
    "4-button_basic",
...
var maleTotal =
  countItems(maleForm1) *
  countItems(maleForm2) *
  countItems(maleForm3) *
  countItems(maleForm4) *
  countItems(maleForm5) *
  countItems(maleForm6) *
  skinTones.length;
var femaleTotal =
  countItems(femaleForm1) *
  countItems(femaleForm2) *
  countItems(femaleForm3) *
  countItems(femaleForm4) *
  countItems(femaleForm5) *
  countItems(femaleForm6) *
  skinTones.length;
consolelog(
  "Hey ! welcome, you have  " + (maleTotal + femaleTotal) + " possibilities"
);
...
function getHandPositionKeywords(selectedOption) {
  ...
  if (sex === "f") {
    if (
      selectedOption === "book_address" ||
      selectedOption === "camera" ||
      selectedOption === "camera_photo" ||
      selectedOption === "boom_mic" ||
      selectedOption === "ipad" ||
      selectedOption === "pad"
    ) {
      position = "grip";
...
function getNailsLayers(option) {
  ...
  nailsLayers = nailsLayers.concat([
    "nails_" + option + "_right_" + handPosition,
  ]);
...
function clearCharacter() {
  ...
  var toBeRemovedList = document.querySelectorAll(
    "#svg1 .character-container > g"
  );
  var counter = toBeRemovedList.length;

  while (counter--) {
    if (
      toBeRemovedList[counter].id != "male_silhouette" &&
      toBeRemovedList[counter].id != "female_silhouette"
    ) {
...
function findNextLayerInDom(item) {
  ...
  while (nextLayerSibling === null) {
    nextLayerSibling = svgContainer.querySelector(
      "#" + layers[itemPosition + 1]
    );
...
function choicesToLayers(c, multiLayer) {
  ...
  for (var index in c.choices) {
    if (index.slice(-5) != "Color") {
      ...
      if (
        index + "_" + c.choices[index] === "body_athletic" ||
        index + "_" + c.choices[index] === "body_default" ||
        index + "_" + c.choices[index] === "body_veiny" ||
        index + "_" + c.choices[index] === "body_android-00"
      ) {
        // TODO replace "c" variable with currentCharacter variable
  ...
  if (c.choices.sex === "f") {
    ...
    selectedLayers.push(
      "nails_short_right_" + handPositionSuffix,
      "nails_short_left_default"
    );
  }
...
function modCharacter(myKey, myValue) {
  ...
  if (
    ...
    currentUser.cc &&
    currentUser.cc.personnages &&
    currentUser.cc.personnageActuel
  ) {
    currentUser.cc.personnages[currentUser.cc.personnageActuel][myKey] =
      myValue;
  }
}
  ...
  for (section in sections) {
    options.forEach(function (d, i) {
      ...
      if (selectedOption != "" && d === selectedOption) {
        ...
        if (
          sections[section] === "brows" ||
          sections[section] === "eyes" ||
          sections[section] === "mouth" ||
          sections[section] === "lashes" ||
          sections[section] === "sockets"
        ) {
...
function sectionShow(multiLayer, id) {
  ...
  if (id.slice(1, 6) === "nails") {
    ...
    svgContainer.querySelector(id + "_left_default").style.pointerEvents =
      "auto";
    svgContainer.querySelector(
      id + "_right_" + positionKeyword
    ).style.opacity = 1;
    svgContainer.querySelector(
      id + "_right_" + positionKeyword
...
var fetchDb = (function () {
  ...
  binary.forEach(function (m) {
    fetchDb[m] = function (path) {
      return fetchDb(path, m);
    };
  });
  ternary.forEach(function (m) {
    fetchDb[m] = function (path, body) {
      return fetchDb(path, body, m);
    };
  });
...
function deleteDbSession() {
  return fetchDb.delete("session").then(function (resp) {
    return resp.json();
  });
}
...
function getDbUser(username) {
  return fetchDb("users/" + username).then(function (resp) {
    return resp.json();
  });
}

function updateDbUser(user) {
  return fetchDb.post("users", user).then(function (resp) {
    if (resp.ok) {
      return resp.json();
    }
  ...
  updateDbUser(currentUser)
    ...
    .then(function (json) {
      ...
      gaga("send", "event", {
        eventCategory: "Navigation",
        eventAction: "Edit",
        eventLabel: "Edit existing character",
      });
    })
    .catch(function (err) {
      consolelog("err", err);
...
function resetCharacters() {
  var charUI = document.querySelector(".js-character-list");
  var charCards = charUI.querySelectorAll(
    ".overlay__char-card:not(.overlay__char-card--orig):not(.overlay__char-new)"
  );
  ...
  createDbUser(username, password, email)
    ...
    .then(function (user) {
      ...
      gaga("send", "event", {
        ...
        eventLabel: "Successfuly Registered account",
      });
    })
    .catch(function (err) {
...
const femaleBody = fetch(
  "/layer/female/" + window.femaleBodyPositionFolder + "/layers.json"
)
  .then(function (res) {
    return res.json();
  })
...
const maleBody = fetch(
  "/layer/male/" + window.maleBodyPositionFolder + "/layers.json"
)
  .then(function (res) {
    return res.json();
  })
...
Promise.all([maleHead, maleBody]).then(function ([maleHead, maleBody]) {
  window.layersMale = assembleLayers(
    maleBody,
    maleHead,
    window.maleBodyPositionFolder,
    "head_front_default"
  );
...
Promise.all([femaleHead, femaleBody]).then(function ([femaleHead, femaleBody]) {
  window.layersFemale = assembleLayers(
    femaleBody,
    femaleHead,
    window.femaleBodyPositionFolder,
    "head_front_default"
  );
...
window.onload = function () {
  ...
  if (homeBtn && typeof gotoNewChar === "function") {
    homeBtn.addEventListener("click", gotoNewChar, false);
  }
  if (newBtn && typeof gotoNewChar === "function") {
    newBtn.addEventListener("click", gotoNewChar, false);
  }
  if (aboutBtn && typeof showAbout === "function") {
    aboutBtn.addEventListener("click", showAbout, false);
  }
  if (faqBtn && typeof showFAQ === "function") {
    faqBtn.addEventListener("click", showFAQ, false);
  }
  if (shopBtn && typeof showShop === "function") {
    shopBtn.addEventListener("click", showShop, false);
  }
  if (whoBtn && typeof whoami === "function") {
    whoBtn.addEventListener("click", whoami, false);
  }
  if (logoutBtn && typeof logout === "function") {
    logoutBtn.addEventListener("click", logout, false);
  }
  if (loginBtn && typeof loginMenu === "function") {
    loginBtn.addEventListener("click", loginMenu, false);
  }
  if (registerBtn && typeof registerMenu === "function") {
    registerBtn.addEventListener("click", registerMenu, false);
  }
  if (registerLink && typeof registerMenu === "function") {
    registerLink.addEventListener("click", registerMenu, false);
  }
  if (creditsBtn && typeof rollCredits === "function") {
    creditsBtn.addEventListener("click", rollCredits, false);
  }
  if (hamburgerBtn && typeof hamburger === "function") {
    hamburgerBtn.addEventListener("click", hamburger, false);
  }
  if (zoomBtn && typeof viewBoxZoom === "function") {
    zoomBtn.addEventListener("change", viewBoxZoom, false);
  }
  if (maleSilhouette && typeof selectMale === "function") {
    maleSilhouette.addEventListener("click", selectMale, false);
  }
  if (femaleSilhouette && typeof selectFemale === "function") {
    femaleSilhouette.addEventListener("click", selectFemale, false);
  }
  if (svgContainer && typeof clickSelect === "function") {
    svgContainer.addEventListener("click", clickSelect, false);
  }
  if (svgContainer && typeof layerHighlight === "function") {
    svgContainer.addEventListener("mouseover", layerHighlight, false);
  }
  if (background && typeof backgroundZoomFull === "function") {
    background.addEventListener("click", backgroundZoomFull, false);
  }
  if (patreonLink && typeof tattle === "function") {
    patreonLink.addEventListener("click", tattle, false);
  }
  if (patreonBtn && typeof gotoPatreon === "function") {
    patreonBtn.addEventListener("click", gotoPatreon, false);
  }
  // if (braveAffiliateBtn && typeof gotoBrave === 'function') { braveAffiliateBtn.addEventListener('click', gotoBrave, false) }
  if (newCharBtn && typeof gotoNewChar === "function") {
    newCharBtn.addEventListener("click", gotoNewChar, false);
  }
  if (loadCharBtn && typeof gotoLoadChar === "function") {
    loadCharBtn.addEventListener("click", gotoLoadChar, false);
  }
  if (nightModeBtn && typeof switchNightMode === "function") {
    nightModeBtn.addEventListener("click", switchNightMode, false);
  }
  if (bigRedBtn && typeof createDecentRandomCharacter === "function") {
    bigRedBtn.addEventListener("click", createDecentRandomCharacter, false);
  }
  if (downloadBtn && typeof download === "function") {
    downloadBtn.addEventListener("click", download, false);
  }
  if (nftBtn && typeof nftModal === "function") {
    nftBtn.addEventListener("click", nftModal, false);
  }
  if (headSizeBtn && typeof resizeHead === "function") {
    headSizeBtn.addEventListener("input", resizeHead, false);
  }
...
function switchNightMode(ev) {
  gaga("send", "event", {
    eventCategory: "Navigation",
    eventAction: "Dark/Light",
    eventLabel: "Switch between Dark mode and Light mode",
  });
  ev.preventDefault();
  var body = document.querySelector("BODY");
  hamburger();
...
function tattle() {
  gaga("send", "event", {
    eventCategory: "Conversion",
    eventAction: "Navbar | Patreon",
    eventLabel: "Open Patreon page from the Navbar/Hamburger menu.",
  });
}
...
function gotoPatreon(evt) {
  ...
  gaga("send", "event", {
    eventCategory: "Conversion",
    eventAction: "Caboose | Patreon",
    eventLabel: "Open Patreon page from Caboose modal.",
  });
  closeAllOverlays();
  setTimeout(function () {
    window.open("https://www.patreon.com/charactercreator");
  }, 500);
}
...
function gotoNewChar(evt) {
  ...
  gaga("send", "event", {
    eventCategory: "Conversion",
    eventAction: "Caboose | New character",
    eventLabel: "Reset to new character from Caboose.",
  });
  closeAllOverlays();
...
function removeGroups() {
  ...
  while (counter--) {
    if (
      groups[counter].id != "female_silhouette" &&
      groups[counter].id != "male_silhouette"
    ) {
...
function clickSelect(ev) {
  ...
  var sectionList = document.querySelectorAll(
    "section.accordeon__section-label"
  );
  var isClosed;
  var sectionLabel;
  var prefix;
  ...
  if (formSection > -1) {
    sectionLabel = sectionList[formSection].querySelector(
      ".accordeon__section-title__text"
    ).innerHTML;

    sectionZoom(sectionLabel);

    isClosed =
      sectionList[formSection].nextSibling.classList.contains("section--hide");
    ...
    if (prefixIndex > -1) {
      itemButtonList =
        sectionList[formSection].nextSibling.querySelectorAll("li.sbl__option");
...
function getHairLayers() {
  ...
  while (counter--) {
    if (
      layers[counter].slice(0, 5) === "hair_" ||
      layers[counter].slice(0, 11) === "facialhair_" ||
      layers[counter].slice(0, 6) === "brows_" ||
      layers[counter].slice(0, 7) === "lashes_"
    ) {
...
function getSkinLayers() {
  ...
  while (counter--) {
    if (
      layers[counter].slice(0, 5) === "body_" ||
      layers[counter].slice(0, 5) === "ears_" ||
      layers[counter].slice(0, 5) === "nose_" ||
      layers[counter].slice(0, 9) === "freckles_" ||
      layers[counter].slice(0, 5) === "scar_" ||
      layers[counter].slice(0, 11) === "wings_devil" ||
      layers[counter].slice(0, 6) === "mouth_" ||
      layers[counter].slice(0, 8) === "sockets_"
    ) {
...
function selectMale(event) {
  ...
  if (event) {
    gaga("send", "event", {
      eventCategory: "Navigation",
      eventAction: "Male",
      eventLabel: "Select male template",
    });
  }
...
function selectFemale(event) {
  ...
  if (event) {
    gaga("send", "event", {
      eventCategory: "Navigation",
      eventAction: "Female",
      eventLabel: "Select female template",
    });
  }
...
function showDoneOptions(ev) {
  ...
  gaga("send", "event", {
    eventCategory: "Conversion",
    eventAction: "Done BTN",
    eventLabel: "Clicked on Done button.",
  });
...
function clearInputUsername() {
  var currentOverlay = document.querySelector(".overlay--show");
  var inputUsername = currentOverlay.querySelectorAll(
    ".overlay__input__username"
  );
...
function removeFromNew(category, section, item) {
  window.localStorage.setItem(
    ["seen-item", window.c.choices.sex, category, section, item].join("---"),
    "seen"
  );
}
...
function isNewInSection(cat, section) {
  ...
  while (verdict === false && counter--) {
    if (
      cat === capitalizeFirstLetter(newItems[counter][0]) &&
      section === capitalizeFirstLetter(newItems[counter][1])
    ) {
      verdict = true;
    }
  }
  ...
  while (verdict === false && counter--) {
    if (
      capitalizeFirstLetter(section) ===
        capitalizeFirstLetter(newItems[counter][1]) &&
      item === newItems[counter][2]
    ) {
      verdict = true;
    }
  }
  ...
  while (formsCounter--) {
    ...
    for (var x in forms[f]) {
      ...
      if (id in skinLayers || section === "body") {
        section = "skin";
      } else if (id in hairLayers || section === "hair") {
        section = "hair";
      }
...
function createDecentRandomCharacter() {
  ...
  hideTutorial();
  gaga("send", "event", {
    eventCategory: "Secret function",
    eventAction: "smartRandomSingle()",
    eventLabel: "Secret Patreon reveal for new random function.",
  });
  hash.clear();
  resetCharacter();
...
function applyHeadResize(headSize) {
  ...
  function stepHeadResize(timestamp) {
    ...
    if (headSize > currentHeadSize) {
      animHeadSize =
        Number(currentHeadSize) +
        (Number(headSize) - Number(currentHeadSize)) * (elapsed / animDur);
    } else {
      animHeadSize =
        Number(headSize) +
        (Number(currentHeadSize) - Number(headSize)) * (1 - elapsed / animDur);
    }
    ...
    while (counter--) {
      headElements[counter].setAttribute(
        "transform",
        "matrix(" +
          scaleValue +
          ",0,0," +
    ...
    if (elapsed < animDur) {
      // Stop the animation after the set duration
...
const showSidebarLeft = () =>
  document.querySelector("#sidebar-left").classList.add("visible");

const hideSidebarLeft = () =>
  document.querySelector("#sidebar-left").classList.remove("visible");
...
function openThumbsLogic(_) {
  ...
  if (
    thumbSectionBtn.classList === undefined &&
    thumbSectionBtn.previousSibling.classList != undefined
  ) {
...
function populateThumbs(svgObject) {
  var emotion =
    document.querySelector("#content_1 .selected--option").classList[2] ===
    "options__emotion";
  ...
  if (layerID.slice(-5, -1) === "_of_") {
    ...
    if (groupRank === groupTotal || !parentEl.firstChild) {
      ...
      if (groupRank > groupInPlace[groupInPlace.length - 1].id.slice(-6, -5)) {
        document
          .querySelector("#content_1 ." + layerID)
          .appendChild(thumbObject);
      }
    }
  } else if (emotion) {
    ...
    if (layerID != "eyeballs_default") {
      document
        .querySelector(
          "#content_1 " + ".emotion_" + splitArray[splitArray.length - 1]
        )
        .appendChild(thumbObject);
    }
    ...
    if (layerID.slice(-5) === "fault") {
      document
        .querySelector("#content_1 ." + "body_default")
        .appendChild(thumbObject);
    } else if (layerID.slice(-5) === "letic") {
      document
        .querySelector("#content_1 ." + "body_athletic")
        .appendChild(thumbObject);
    } else if (layerID.slice(-5) === "veiny") {
      document
        .querySelector("#content_1 ." + "body_veiny")
        .appendChild(thumbObject);
    } else if (layerID.slice(-5) === "id-00") {
      document
        .querySelector("#content_1 ." + "body_android-00")
        .appendChild(thumbObject);
    }
  } else {
    if (layerID === "eyeball_right") {
      ...
      if (openedDrawer.contains("options__pupils")) {
        while (counter--) {
          pupilObject = showPupilObject(
            thumbObject,
            pupilShapeList[counter]
          ).cloneNode(true);

          document
            .querySelector("#content_1 .pupils_" + pupilShapeList[counter])
            .appendChild(pupilObject);
        }
      }
...
function tutorialPosition(step) {
  ...
  if (step === "random") {
    ...
    el.style.top =
      randomBtn.getBoundingClientRect().top - bbox.height - 20 + "px";
    el.style.left = w / 2 - bbox.width / 2 + "px";
  }
...
function getViewBox(t, d) {
  ...
  if (sex === "m") {
    var idDict = {
      ...
      holding_camera: "184 306 74 74",
      holding_book_address: "195 292 60 60",
      holding_ipad: "195 292 60 60",
      holding_pad: "195 292 60 60",
    ...
    var idDict = {
      ...
      holding_camera: "165 306 80 80",
      holding_book_address: "177 290 60 60",
      holding_ipad: "177 290 60 60",
      holding_pad: "177 290 60 60",
...
function sectionZoom(sectionLabel) {
  ...
  if (sectionLabel === "Head") {
    zoomInput.value = 3;
    zoomFace();
  }
  if (sectionLabel === "Accessories") {
    zoomInput.value = 1;
    zoomArms();
  }
  if (sectionLabel === "Torso") {
    zoomInput.value = 2;
    zoomTorso();
  }
  if (sectionLabel === "Body") {
    zoomInput.value = 1;
    zoomBody();
  }
  if (sectionLabel === "Legs") {
    zoomInput.value = 4;
    zoomLegs();
  }
  if (sectionLabel === "Feet") {
    zoomInput.value = 5;
    zoomFeet();
  }
}
...
function animateZoom(newViewBox) {
  ...
  if (
    newViewBox != "10 50 540 540" &&
    !characterSVG.classList.contains("zoomed")
  ) {
...
function addColorPicker() {
  var e = document.querySelector(".section--selected").innerHTML.toLowerCase();
  getColor(e), showTutorial("random");
}

function hideColorPicker() {
  var e = document.querySelector(".colorpicker-wrapper");
  e &&
    !e.classList.contains("section--hide") &&
    e.classList.add("section--hide");
}
function myFunction() {
